"""Definition of the Sparql Archetypes content type
"""
...
from eea.sparql.interfaces import ISparqlArchetypes, ISparqlBookmarksFolderArchetypes
...
class SparqlArchetypes(base.ATCTContent, ZSPARQLMethod):
    """Sparql"""
    # implements(ISparqlArchetypes, IVersionEnhanced)
    implements(ISparqlArchetypes)

    meta_type = "SparqlArchetypes"
    schema = SparqlSchema
...
def updateOtherCachedFormats(obj, scheduled_at, endpoint, query,
                                    _type, accept):
    """ Async that updates json, xml, xmlschema exports
    """
...
class SparqlArchetypesBookmarksFolder(ATFolder, Sparql):
    """Sparql Bookmarks Folder"""
    implements(ISparqlBookmarksFolderArchetypes)
    meta_type = "SparqlArchetypesBookmarksFolder"
    ...
    def checkQuery(self, title, endpoint, query):
        ...
        for sparql in self.values():
            if sparql.title == title.encode('utf8'):
                found = True
                break
                # TODO: eea.versions not ported to py3 yet
    ...
    def addOrUpdateQuery(self, title, endpoint, query):
        ...
        if not ob:
            _id = generateUniqueId("SparqlArchetypes")
            _id = self.invokeFactory(type_name="SparqlArchetypes", id=_id)
            ob = self[_id]
            ob.edit(
                title=title,
            ...
            # ob.invalidateWorkingResult() #TODO: async no longer available
        else:
            if changed:
...
atapi.registerType(SparqlArchetypes, PROJECTNAME)
atapi.registerType(SparqlArchetypesBookmarksFolder, PROJECTNAME)
